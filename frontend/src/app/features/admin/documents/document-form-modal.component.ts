...
/** Modal de creación de documento para administración (sin restricción de autor). */
@Component({
  ...
  template: `
    <app-modal [open]="opened()" (close)="handleClose()">
      ...
      <form [formGroup]="form" (ngSubmit)="onSubmit()" class="space-y-4">
        ...
        <div class="space-y-1.5">
          <label appLabel for="doc-file-admin">Archivo adjunto <span class="text-slate-400 font-normal">(opcional · PDF, DOCX, PNG, JPG · máx 10 MB)</span></label>
          <div class="flex items-center gap-3">
            <label class="flex items-center gap-2 cursor-pointer rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 hover:bg-slate-50 transition-colors">
              ...
              <input id="doc-file-admin" type="file" class="sr-only" accept=".pdf,.docx,image/png,image/jpeg" (change)="onFileChange($event)" />
            </label>
            @if (selectedFile()) {
...
export class AdminDocumentFormModalComponent {
