...
{ic['n_with_707']:,} de {r['n_filtered']:,} registros ({ic['n_with_707']/r['n_filtered']*100:.2f} %). El
...
demostrado con {r['n_filtered']:,} registros e intervalos de confianza propios, no
...
- **El NPDB no es fuente válida para calibrar `informed_consent` ni
  `has_prior_complaints`.** Demostrado con {r['n_filtered']:,} registros e intervalos
...
1. El NPDB no es fuente válida para calibrar `informed_consent` ni
   `has_prior_complaints` — y eso queda demostrado con {r['n_filtered']:,} registros e
...
3. `time_since_incident_days` sí resultó consistente en dirección
   (r = {fmt(ts['pearson_corr_lag_years_vs_outcome'])}), aunque débil y con el desajuste de dominio
   documentado en §4.
